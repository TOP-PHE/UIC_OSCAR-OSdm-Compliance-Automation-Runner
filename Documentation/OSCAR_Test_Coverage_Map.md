...
Independent suite that may run before SALE. Two-layer compliance: **Layer 1** (`osdmCompliance.js` — structural shape, required fields, types, enums) + **Layer 2** (`osdmSchema.js` → `validateSchema(<Component>, body, {endpoint})` — deep AJV-style schema match against the version-matched OSDM JSON Schema). A shared `handleSystemInfoStatus()` classifier (`library-bruno/osdmCompliance.js:609-652`) version-gates 404s: an endpoint introduced after the declared OSDM version returns 404 → reported as `out of scope` (skip, not fail).
...
- `provisionalPrice` + `confirmedPrice` exist with `{currency, scale}`; `provisionalPrice.currency === confirmedPrice.currency` (B4); H3 cross-flow: `booking.provisionalPrice.currency === offerCurrency` from step 01
...
**Response checks**: HTTP 200; `jsonData.passenger` present; runs `patchMultiPassengerResponse` for the GET shape; loops per-passenger.
...
| 11 | `GET /bookings/{bookingId}/refund-offers/{refundOffersOfferId}` | `11. GET Refund Offer.yml` | refund-offer id | Same `postPatchRefundOfferResponse` pipeline expecting `["PROPOSED"]` |
| 12 | `GET /bookings/{bookingId}?embed=ALL` | `12. GET Booking before Patch Refund.yml` | – | `postCreateBookingResponse` re-validation with status expectations |
...
| 12 | `GET /bookings/{bookingId}/exchange-operations/{exchangeOperationId}` | `12. GET Exchange Offer.yml` | exchangeOperationId path | `postPatchExchangeOperationsResponse(…)` re-validation; also captures admission/reservation/ancillary part IDs |
| 13 | `GET /bookings/{bookingId}?embed=ALL` | `13. GET Booking before Fulfillment.yml` | – | Booking re-validation |
...
| 15 | `GET /bookings/{bookingId}?embed=ALL` | `15. GET Booking after Fulfillment.yml` | – | Booking re-validation |
...
| 17 | `GET /bookings/{bookingId}?embed=ALL` | `17. GET Booking after Fulfillment.yml` | – | Final booking validation |
...
| Status-classification failures for System-Information | `osdmCompliance.js::classifySystemInfoStatus:609-632` (`01-System Infos Requests/*`) | 401/403/404(when endpoint applicable)/5xx all generate FAILING `GET <endpoint> → <status>` assertions with specific failure messages (e.g. "Insufficient permissions", "Endpoint expected in OSDM 3.X but not implemented by this vendor") |
...
| ProvisionalPrice missing on booking | `library-bruno/bookings.js:502-509` | `provisionalPrice missing` / `confirmedPrice missing` |
